...
CREATE TABLE viewgroup_templates (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
...
CREATE INDEX idx_viewgroup_templates_project ON viewgroup_templates(project_id);
CREATE INDEX idx_viewgroup_templates_workspace ON viewgroup_templates(workspace_id);
CREATE INDEX idx_viewgroup_templates_owner ON viewgroup_templates(owner_id);
CREATE INDEX idx_viewgroup_templates_scope ON viewgroup_templates(scope);
...
CREATE TRIGGER update_viewgroup_templates_updated_at BEFORE UPDATE ON viewgroup_templates FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
