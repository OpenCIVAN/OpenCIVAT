...
## ⚠️ CRITICAL: Use NEW PanelShell Architecture

**CanvasMapPanel should use the NEW PanelShell system** - NOT the old LeftPanel tab pattern.

| Question | Answer |
|----------|--------|
| **What system to use?** | `PanelShell` (new floating-first) |
| **Where to put files?** | `panels/CanvasMapPanel/` |
| **What chrome level?** | `CHROME_LEVELS.FULL` |
| **Use LeftPanel tabs?** | ❌ NO (legacy, being phased out) |
| **Use FloatingPanel?** | ❌ NO (legacy, being replaced) |
| **Pattern to follow?** | See `PanelShell.stories.jsx` |
...
See **Canvas_Map_Panel_Architecture_Clarification.md** and **PanelShell_Unified_Workspace_Design_Session_Memory_Log.md** for full details.
...
├── CanvasMapPanel.jsx           # PanelShell wrapper
├── CanvasMapPanel.scss          # Styles
├── CanvasMapContent.jsx         # Inner content (receives sizeMode)
├── components/
│   ├── ModeTabBar/              # Navigate|Layout|Links|Team tabs
│   ├── ModeToolbar/             # Mode-specific toolbar controls
│   ├── Minimap/                 # The grid visualization
│   │   ├── MinimapGrid.jsx      # Grid container with labels
│   │   ├── VGBlock.jsx          # ViewGroup rendering
│   │   ├── ViewCell.jsx         # Individual view (View mode)
│   │   ├── ViewportIndicator.jsx
│   │   ├── CollaboratorIndicator.jsx
│   │   └── LinkLines.jsx        # SVG link visualization
│   ├── panels/                  # Contextual content below minimap
│   │   ├── NavigatePanel.jsx    # Quick nav + bookmarks
│   │   ├── LayoutPanel.jsx      # VG list + inactive
│   │   ├── LinksPanel.jsx       # VG links + View links
│   │   └── CollaboratePanel.jsx # Me + Team sub-tabs
...
│       └── LinkItem.jsx
└── hooks/
    ├── useCanvasMapState.js     # Mode, selection, visibility state
    ├── useFlattenedViews.js     # View mode position calculation
    └── useGridLabels.js         # Excel-style A1, B2 formatting
```

### Component Hierarchy (PanelShell Pattern)

```jsx
// CanvasMapPanel.jsx - The PanelShell wrapper
import { PanelShell, CHROME_LEVELS } from '@UI/react/components/panels/PanelShell';
...
```jsx
// CanvasMapContent.jsx - The actual panel content
export function CanvasMapContent({ width, height, sizeMode }) {
  const [mapMode, setMapMode] = useState('navigate');
  
  return (
    <div className="canvas-map-content">
      {/* Mode tabs */}
      <ModeTabBar mode={mapMode} onModeChange={setMapMode} />
      
      {/* Mode-specific toolbar */}
      <ModeToolbar mode={mapMode} sizeMode={sizeMode} />
      
      {/* Always visible minimap */}
      <Minimap sizeMode={sizeMode} />
      
      {/* Mode-specific content panel */}
      <div className="canvas-map-content__panel">
        {mapMode === 'navigate' && <NavigatePanel sizeMode={sizeMode} />}
        {mapMode === 'layout' && <LayoutPanel sizeMode={sizeMode} />}
        {mapMode === 'links' && <LinksPanel sizeMode={sizeMode} />}
        {mapMode === 'collaborate' && <CollaboratePanel sizeMode={sizeMode} />}
      </div>
