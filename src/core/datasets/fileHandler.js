...
import { createOrientationMarker } from "@Core/scene/orientationMarker.js";
import {
  getSceneObjects,
...
import { logMemoryUsage } from "@Services/tensorflow/tensorflowSetup.js";
import {
  logProgress,
  logSuccess,
  logWarning,
...
async function handleFile(e) {
  ...
  if (files.length > 0) {
    ...
    try {
      ...
      const { visualizationManager } = await import(
        "@Core/visualizationManager.js"
      );
      ...
      while (attempts < maxAttempts) {
        ...
        if (dataset && dataset.polydata) {
          ...
          import("@Core/scene/sceneManager.js").then(
            ({ forceInitialRender }) => {
              forceInitialRender();
            }
          );
