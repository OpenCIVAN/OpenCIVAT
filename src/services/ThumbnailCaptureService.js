...
class ThumbnailCaptureService {
    ...
    simplifySVG(svg) {
        ...
        const interactiveAttrs = ['onclick', 'onmouseover', 'onmouseout', 'onmousedown', 'onmouseup'];
        svg.querySelectorAll('*').forEach((el) => {
            interactiveAttrs.forEach((attr) => el.removeAttribute(attr));
        });

        // Remove hidden elements
        svg.querySelectorAll('[visibility="hidden"], [display="none"], [opacity="0"]')
        ...
        svg.querySelectorAll('[id], [class]').forEach((el) => {
            ...
            if (id && !svg.querySelector(`[href="#${id}"], [xlink\\:href="#${id}"]`)) {
     ...
     */
    toDataUrl(thumbnail) {
        const mimeType = thumbnail.format === 'svg' ? 'image/svg+xml' :
