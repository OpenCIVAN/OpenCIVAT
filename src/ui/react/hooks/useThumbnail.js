...
export function useThumbnail(viewId, options = {}) {
  ...
  const fetchThumbnail = useCallback(async () => {
    ...
    try {
      ...
      log.debug(`Fetching thumbnail: ${endpoint}`);
      ...
      if (response.status === 404) {
        ...
        if (retryInterval > 0 && retryCountRef.current < maxRetries) {
          ...
          retryTimeoutRef.current = setTimeout(fetchThumbnail, retryInterval);
        }
  ...
  useEffect(() => {
    ...
    const handleThumbnailReady = (event) => {
      ...
      if (isMatch) {
        log.debug(`Server generated thumbnail for ${viewId}, fetching...`);
