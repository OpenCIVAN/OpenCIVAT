...
export function useRoomsTab(options = {}) {
  ...
  const sessionProjectId = sessionManager.getProjectId?.();
  const sessionRoomId = sessionManager.getRoomId?.();
  ...
  useEffect(() => {
    ...
    log.debug("useRoomsTab initialized with projectId:", {
      ...
      fromSessionManager: sessionProjectId,
      fromRoomId: sessionRoomId,
      final: projectId,
    });
  }, [projectId, propProjectId, sessionProjectId, sessionRoomId]);
